...
with tempfile.TemporaryDirectory(prefix='omp-verify-') as scratch:
    ...
    result = subprocess.run([executable, '--mode', 'rpc', '--no-session', '--no-title', '--no-lsp', '--cwd', str(cwd)],
    ...
    summary = {
        ...
        'custom_extensions': sorted(p.name for p in (Path.home() / '.omp/agent/extensions').glob('*')),
        'tools': tools,
