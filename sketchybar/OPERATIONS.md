...
While the topology stays healthy, a transiently failing windows subquery (for example during a Space-switch animation) retains the last accepted app ligatures instead of blanking them; only an accepted response replaces state. On a Space switch, the previously focused app never bleeds onto the newly selected Space while the replacement windows query is in flight.
