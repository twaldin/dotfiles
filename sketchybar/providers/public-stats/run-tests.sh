...
[[ ${#test_sources[@]} -eq 6 ]] || { echo E_TEST_SOURCE_COUNT >&2; exit 1; }

run_suite() {
  local output=$1
  shift
  xcrun swiftc     "$@"     -warnings-as-errors     -parse-as-library     -swift-version 6     -target arm64-apple-macosx15.0     "${sources[@]}"     "${test_sources[@]}"     -framework AppKit     -framework DiskArbitration     -framework IOKit     -framework Metal     -framework Network     -framework SystemConfiguration     -o "$output"
  local result
  result=$("$output")
  [[ "$result" == '60 TESTS PASSED' ]] || { echo E_TEST_RESULT >&2; exit 1; }
}
