...
check(date_item["geometry"]["width"] == 148, "date width")
layout_text = (root / "lib/calendar_bar_layout.lua").read_text()
advance_match = re.search(r"title_narrow_advance\s*=\s*([0-9.]+)", layout_text)
item_match = re.search(r"item\s*=\s*([0-9.]+)", settings_text)
check(advance_match is not None and item_match is not None
      and "content_gap = settings.spacing.item" in layout_text,
      "shared date layout constants")
advance, gap = float(advance_match.group(1)), float(item_match.group(1))
...
# is the exact 148-point authority; each reported lane may lose less than one
# point to query serialization.
ideal_margin = (148 - gap - (len(date_text) + len(time_text)) * advance) / 2
...
check(abs(expected_icon_width + expected_label_width - 148) < 0.001, "date ideal lane sum")
check(abs(icon["width"] - expected_icon_width) < 1 and abs(label["width"] - expected_label_width) < 1, "date serialized lane quantization")
check(abs(icon["width"] + label["width"] - 148) <= 1, "date serialized lane sum")
...
for display in sorted(event_rects):
    ...
    check(1 <= event_width <= 188 and event_rect["size"][1] == 36.0, "bounded dynamic event rectangle")
    check(date_rect["size"] == [148.0, 36.0], "date rectangle")
    check(system_rect["size"][1] == 36.0 and 1 <= system_rect["size"][0] <= 256,
          "bounded left control rectangle")
    check(event_rect["origin"][0] + event_width == date_rect["origin"][0], "event/date continuity")
    check(event_width + 148 <= 336, "touching calendar maximum")

def surface_color(item, idle_color, label):
    background = item["geometry"]["background"]
    check(background["drawing"] == "on", f"{label} drawing")
    check(background["height"] == 28 and background["corner_radius"] == 0, f"{label} sharp shape")
...
print(json.dumps({
    ...
    "date_group": 148,
    "system_group": "bounded-left-controls",
    "calendar_limit": 336,
    "date_centered": True,
    "date_gap": 4,
    "continuous": True,
